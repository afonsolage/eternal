use bevy::{
    ...
    reflect::{
        FromReflect, GetTypeRegistration, Reflect, TypeRegistry, serde::TypedReflectDeserializer,
    },
};
use noise::{
...
pub enum NoiseStackError {
    ...
    SpecNoMain,
    #[error("Failed to build noise stack: Spec {0} was not found")]
    SpecNotFound(String),
...
impl NoiseStack {
    #[cfg(test)]
    fn load(path: impl AsRef<std::path::Path>) -> Result<Self, NoiseStackError> {
...
mod tests {
    ...
    #[test]
    fn load() {
        let path = format!("{}/config/procgen/atlas.ron", env!("ASSETS_PATH"));
